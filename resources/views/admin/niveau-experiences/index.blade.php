...
@section('header', 'Gestion des niveaux d\'expérience')
...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6 animate-fade-in-up delay-1">
        ...
        <a href="{{ route('admin.parametres.index') }}" 
           class="px-4 py-2 border border-gray-200 rounded-lg text-gray-700 bg-white hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-2">
    ...
    <div class="max-w-2xl mx-auto animate-fade-in-up delay-1">
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="p-5">
                <form method="POST" action="{{ route('admin.niveau-experiences.store') }}" class="flex gap-2">
                    @csrf
                    <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 flex-1">
                        ...
                        <input type="text" name="nom" placeholder="Ex: Junior, Confirmé, Senior..." 
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="bg-gradient-to-r from-[#970d0d] to-[#7a0a0a] px-6 py-4">
                <h2 class="text-lg font-semibold text-white flex items-center">
                    ...
                    Niveaux d'expérience existants
                </h2>
            </div>
            <div class="p-5">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="table-row">
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $ne->nom }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        <!-- Formulaire de modification inline -->
                                        <form action="{{ route('admin.niveau-experiences.update', $ne) }}" method="POST" class="inline">
                                            @csrf @method('PUT')
                                            <div class="flex items-center space-x-1">
                                                <input type="text" name="nom" value="{{ $ne->nom }}" 
                                        ...
                                        <form action="{{ route('admin.niveau-experiences.destroy', $ne) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer ce niveau d\'expérience ?')">
                                            @csrf @method('DELETE')
                    ...
                    @if($niveauExperiences->isEmpty())
                        <p class="text-center text-gray-500 py-4">Aucun niveau d'expérience enregistré.</p>
                    @endif
                </div>
            </div>
        </div>
