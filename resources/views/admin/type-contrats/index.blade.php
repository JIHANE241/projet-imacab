...
@section('header', 'Gestion des types de contrat')
...
@section('content')
    ...
    <div class="max-w-2xl mx-auto animate-fade-in-up delay-1">
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="p-5">
                <form method="POST" action="{{ route('admin.type-contrats.store') }}" class="flex gap-2">
                    @csrf
                    <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500 flex-1">
                        ...
                        <input type="text" name="nom" placeholder="Ex: CDI, CDD, Stage..." 
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="table-row">
                                <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $tc->nom }}</td>
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-right">
                                    <div class="flex items-center justify-end space-x-2">
                                        <!-- Formulaire de modification inline -->
                                        <form action="{{ route('admin.type-contrats.update', $tc) }}" method="POST" class="inline">
                                            @csrf @method('PUT')
                                            <div class="flex items-center space-x-1">
                                                <input type="text" name="nom" value="{{ $tc->nom }}" 
                                        ...
                                        <form action="{{ route('admin.type-contrats.destroy', $tc) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer ce type de contrat ?')">
                                            @csrf @method('DELETE')
                    ...
                    @if($typeContrats->isEmpty())
                        <p class="text-center text-gray-500 py-4">Aucun type de contrat enregistré.</p>
                    @endif
                </div>
            </div>
        </div>
