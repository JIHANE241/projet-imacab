...
<div class="max-w-4xl mx-auto animate-fade-in-up delay-1">
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-[#970d0d] to-[#7a0a0a] px-6 py-5 rounded-t-xl">
            <h2 class="text-xl font-bold text-white">Modifier l'offre</h2>
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('admin.offres.update', $offre) }}">
                @csrf
                @method('PUT')
                <input type="hidden" name="direction_id" value="{{ $offre->direction_id }}">
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <select name="category_id" id="category_id" required
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('category_id', $offre->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->nom }}</option>
                                @endforeach
                            </select>
                ...
                <div class="mt-6">
                    ...
                    <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                        ...
                        <textarea name="description" id="description" rows="6" required
                                  class="pl-10 pr-4 py-2 w-full border-0 focus:ring-0 focus:outline-none"
                                  placeholder="Détails du poste...">{{ old('description', $offre->description) }}</textarea>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-6">
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <select name="type_contrat_id" id="type_contrat_id" required
                                ...
                                @foreach($typeContrats as $tc)
                                    <option value="{{ $tc->id }}" {{ old('type_contrat_id', $offre->type_contrat_id) == $tc->id ? 'selected' : '' }}>{{ $tc->nom }}</option>
                                @endforeach
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @error('type_contrat_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <select name="niveau_experience_id" id="niveau_experience_id" required
                                    class="pl-10 pr-8 py-2 w-full border-0 focus:ring-0 focus:outline-none appearance-none bg-white">
                                <option value="">Sélectionner</option>
                                @foreach($niveauExperiences as $ne)
                                    <option value="{{ $ne->id }}" {{ old('niveau_experience_id', $offre->niveau_experience_id) == $ne->id ? 'selected' : '' }}>{{ $ne->nom }}</option>
                                @endforeach
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @error('niveau_experience_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <select name="niveau_etude_id" id="niveau_etude_id" required
                                ...
                                @foreach($niveauxEtudes as $ne)
                                    <option value="{{ $ne->id }}" {{ old('niveau_etude_id', $offre->niveau_etude_id) == $ne->id ? 'selected' : '' }}>{{ $ne->nom }}</option>
                                @endforeach
                            </select>
                            <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @error('niveau_etude_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-6">
                    <div>
                        ...
                        @error('date_publication') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <input type="date" name="date_limite" id="date_limite" value="{{ old('date_limite', optional($offre->date_limite)->format('Y-m-d')) }}"
                                   class="pl-10 pr-4 py-2 w-full border-0 focus:ring-0 focus:outline-none">
                        </div>
                        @error('date_limite') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <input type="number" name="salaire_min" id="salaire_min" value="{{ old('salaire_min', $offre->salaire_min) }}"
                                   class="pl-10 pr-4 py-2 w-full border-0 focus:ring-0 focus:outline-none"
                                   placeholder="0">
                        </div>
                        @error('salaire_min') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <input type="number" name="salaire_max" id="salaire_max" value="{{ old('salaire_max', $offre->salaire_max) }}"
                                   class="pl-10 pr-4 py-2 w-full border-0 focus:ring-0 focus:outline-none"
                                   placeholder="0">
                        </div>
                        @error('salaire_max') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="missions" id="missions" rows="4" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#970d0d] focus:border-transparent">{{ old('missions', $offre->missions) }}</textarea>
                </div>

                <div class="mt-6">
                    <label for="profil" class="block text-sm font-medium text-gray-700 mb-1">Profil recherché (optionnel)</label>
                    <textarea name="profil" id="profil" rows="4" class="w-full border border-gray-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#970d0d] focus:border-transparent">{{ old('profil', $offre->profil) }}</textarea>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                        ...
                        <select name="statut" id="statut" required
                            ...
                            <option value="ouverte" {{ old('statut', $offre->statut) == 'ouverte' ? 'selected' : '' }}>Ouverte</option>
                            <option value="fermée" {{ old('statut', $offre->statut) == 'fermée' ? 'selected' : '' }}>Fermée</option>
                        </select>
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
