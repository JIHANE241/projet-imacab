...
<head>
    ...
    <title>IMACAB Candidat - {{ config('app.name') }}</title>
    <!-- Fonts -->
            ...
            <div class="flex-1 overflow-y-auto py-4 px-2 sidebar-scroll">
                <div class="mb-6 animate-fade-in-up delay-2">
                    ...
                    <nav class="mt-2 space-y-0.5">
                        <a href="{{ route('candidat.dashboard') }}" class="nav-link flex items-center {{ request()->routeIs('candidat.dashboard') ? 'nav-link-active' : '' }}">
                            <i class="fas fa-tachometer-alt nav-icon mr-3"></i>
                            <span>Dashboard</span>
                        </a>
                        <a href="{{ route('candidat.offres.index') }}" class="nav-link flex items-center {{ request()->routeIs('candidat.offres.*') ? 'nav-link-active' : '' }}">
                            <i class="fas fa-briefcase nav-icon mr-3"></i>
                            <span>Offres d'emploi</span>
                        </a>
                        <a href="{{ route('candidat.candidatures.index') }}" class="nav-link flex items-center {{ request()->routeIs('candidat.candidatures.*') ? 'nav-link-active' : '' }}">
                            <i class="fas fa-file-alt nav-icon mr-3"></i>
                            <span>Mes candidatures</span>
                        </a>
                        <a href="{{ route('candidat.entretiens.index') }}" class="nav-link flex items-center {{ request()->routeIs('candidat.entretiens.*') ? 'nav-link-active' : '' }}">
                            <i class="fas fa-calendar-check nav-icon mr-3"></i>
                            <span>Mes entretiens</span>
                        </a>
                        <a href="{{ route('candidat.cv.edit') }}" class="nav-link flex items-center {{ request()->routeIs('candidat.cv.*') ? 'nav-link-active' : '' }}">
                            <i class="fas fa-id-card nav-icon mr-3"></i>
                            <span>Mon CV</span>
                        </a>
                        <a href="{{ route('candidat.profil.edit') }}" class="nav-link flex items-center {{ request()->routeIs('candidat.profil.*') ? 'nav-link-active' : '' }}">
            ...
            <div class="border-t border-gray-200 p-4 hover:bg-gray-50 transition-all animate-fade-in-up delay-5">
                <div class="flex items-center group">
                    <div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-full flex items-center justify-center text-white font-semibold text-lg shadow-md transition-transform group-hover:scale-110">
                        @if(Auth::user()->photo)
                            <img src="{{ asset('storage/'.Auth::user()->photo) }}" alt="Photo" class="w-full h-full rounded-full object-cover">
                        @else
                            {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                        @endif
                    </div>
                    <div class="ml-3 flex-1">
                        <p class="text-sm font-medium text-gray-800">{{ Auth::user()->name }} {{ Auth::user()->prenom }}</p>
                                    ...
                                    @forelse(Auth::user()->notifications()->take(10)->get() as $notification)
                                        @php
                                            ...
                                            if ($notificationType === 'nouvelle_candidature' && isset($data['candidature_id'])) {
                                                $targetUrl = route('candidat.candidatures.show', $data['candidature_id']);
                                            } elseif ($notificationType === 'entretien_planifie' && isset($data['entretien_id'])) {
                                                $targetUrl = route('candidat.entretiens.show', $data['entretien_id']);
