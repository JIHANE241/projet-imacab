...
@section('header', 'Planifier un entretien')
...
<div class="max-w-2xl mx-auto animate-fade-in-up delay-1">
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="bg-gradient-to-r from-[#970d0d] to-[#7a0a0a] px-6 py-5 rounded-t-xl">
            <h2 class="text-xl font-bold text-white">Planifier un entretien</h2>
            <p class="text-red-100 text-sm mt-1">Tous les champs marqués d’un astérisque (*) sont obligatoires</p>
        </div>
        <div class="p-6">
            <form method="POST" action="{{ route('responsable.entretiens.store') }}">
                @csrf

                @if($candidature)
                    <!-- Candidature pré-sélectionnée (lecture seule) -->
                    <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-[#970d0d]/10 rounded-full flex items-center justify-center text-[#970d0d]">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                                </svg>
                            </div>
                            <div>
                                <p class="text-sm text-gray-500">Candidature sélectionnée</p>
                                <p class="font-medium text-gray-800">{{ $candidature->candidat->name }} {{ $candidature->candidat->prenom }}</p>
                                <p class="text-sm text-gray-600">{{ $candidature->offre->titre }} · {{ $candidature->offre->direction->nom }}</p>
                            </div>
                        </div>
                        <input type="hidden" name="candidature_id" value="{{ $candidature->id }}">
                    </div>
                @else
                    <!-- Sélecteur de candidature (admin) -->
                    <div>
                        <label for="candidature_id" class="block text-sm font-medium text-gray-700 mb-1">
                            Candidature <span class="text-red-500">*</span>
                        </label>
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <select name="candidature_id" id="candidature_id" required
                                ...
                                @foreach($candidatures as $c)
                                    <option value="{{ $c->id }}" {{ old('candidature_id') == $c->id ? 'selected' : '' }}>
                                        {{ $c->candidat->name }} {{ $c->candidat->prenom }} – {{ $c->offre->titre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if($candidatures->isEmpty())
                            <p class="text-sm text-amber-600 mt-1">Aucune candidature éligible pour un entretien.</p>
                        @endif
                    </div>
                @endif
                ...
                <div class="mt-6">
                    ...
                    <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                        ...
                        <input type="text" name="lieu" id="lieu" value="{{ old('lieu') }}"
                               class="pl-10 pr-4 py-2 w-full border-0 focus:ring-0 focus:outline-none"
                               placeholder="Ex: Salle de réunion 203 ou lien visio">
                    </div>
                    @error('lieu')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mt-6">
                    ...
                    <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                        ...
                        <select name="statut" id="statut" required
                            ...
                            <option value="passe" {{ old('statut') == 'passe' ? 'selected' : '' }}>Passé</option>
