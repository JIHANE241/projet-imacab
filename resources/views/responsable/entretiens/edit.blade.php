...
<div class="max-w-2xl mx-auto animate-fade-in-up delay-1">
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="p-6">
            <!-- Informations de la candidature (lecture seule) -->
            <div class="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">Candidature associée</p>
                        <p class="font-medium text-gray-800">{{ $entretien->candidature->candidat->name }} {{ $entretien->candidature->candidat->prenom }}</p>
                        <p class="text-sm text-gray-600">{{ $entretien->candidature->offre->titre }} · {{ $entretien->candidature->offre->direction->nom }}</p>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('responsable.entretiens.update', $entretien) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <div class="relative input-icon border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500">
                            ...
                            <input type="time" name="heure" id="heure" value="{{ old('heure', $entretien->heure ? $entretien->heure->format('H:i') : '') }}" required
                                   class="pl-10 pr-4 py-2 w-full border-0 focus:ring-0 focus:outline-none">
                        </div>
                        @error('heure')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
